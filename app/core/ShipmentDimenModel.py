...
class ShipmentDimenModel(ResultModel):
    ...
    def get_json_result(self, data, y_pred, y_test, hasActual=True, mode=None):
        ...
        for i in range(length):          
            ...
            width = np.asscalar(np.int64(data.iloc[i]['CustomStdWidth']))
            height = np.asscalar(np.int64(data.iloc[i]['CustomStdHeight']))
            length = np.asscalar(np.int64(data.iloc[i]['CustomStdLength']))
            results.append({
                "location": location,                        
                "month": month, 
                "weight": weight, 
